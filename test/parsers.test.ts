...
describe('parseSearch', () => {
	it('extracts every well-formed result and skips the broken one', async () => {
		const results = await parseSearchHtml(SEARCH_HTML, BASE);
		expect(results).toHaveLength(2);
	});

	it('decodes HTML entities in titles', async () => {
		const [first] = await parseSearchHtml(SEARCH_HTML, BASE);
		// Regression: entities used to reach clients as literal &amp; / &#39;.
		expect(first?.title).toBe("Alpha & Omega's Tale");
	});

	it('derives the slug from the href and resolves relative covers', async () => {
		const [first, second] = await parseSearchHtml(SEARCH_HTML, BASE);
		...
		expect(second?.cover_url).toBe(`${BASE}/media/manga_covers/beta.png`);
	});
	...
	it('returns null rather than NaN when the rating span is absent', async () => {
		const [, second] = await parseSearchHtml(SEARCH_HTML, BASE);
	...
	it('returns an empty list for markup with no results', async () => {
		expect(await parseSearchHtml('<ul class="novel-list"></ul>', BASE)).toEqual([]);
	});
});

describe('parseManhwa', () => {
	it('extracts the core detail fields', async () => {
		const manhwa = await parseManhwaHtml(MANHWA_DETAIL_HTML, 'alpha-tale-x1', BASE);
	...
	it('prefers data-src over the lazy-load placeholder and resolves it', async () => {
		const manhwa = await parseManhwaHtml(MANHWA_DETAIL_HTML, 'alpha-tale-x1', BASE);
		expect(manhwa.cover_url).toBe(`${BASE}/media/manga_covers/alpha.jpg`);
	});
